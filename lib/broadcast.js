class Broadcast {
  ...
  checkBitDistance(neighborGenVal, myGenValue){
    ...
    for(let i = 0;i<tempDeparted.length;i++){
  ...
  /****NEIGHBOR AND CONNECTING FUNCTIONS****/
  ...
  zoneShare() {
    ...
    return zoneOffer; // zoneOffer = [connecting, original]
  }
  ...
  acceptConnRequest(peerId, siteId) {
    ...
    const initialData = JSON.stringify({
      ...
      zoneOffer: zoneOffer[0], // zone of connecting peer
    ...
    connection.on('data', data => {
      ...
      switch(dataObj.type) {
        ...
        case 'askUpdateNeighbor':
          this.replyRequest(dataObj.peerId, dataObj.siteId, dataObj.region, dataObj.genValue);
          break;
        case 'doUpdateNeighbor': // adds neighbor
          let connection = this.outConns.find((conn) => conn.peer === dataObj.peerId);
          let remove = this.updateNeighbor(dataObj.peerId, dataObj.siteId, dataObj.region, dataObj.genValue);
          if(connection && remove){
            connection.close();
          }
          break;
        case 'addNeighbor': // adds neighbor
          this.acceptNeighbor(dataObj.peerId, dataObj.siteId, dataObj.region, dataObj.genValue);
          break;
